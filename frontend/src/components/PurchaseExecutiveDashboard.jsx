...
export default function PurchaseExecutiveDashboard({ currentUser }) {
  ...
  // --- TICKET OPENING LOGIC (ACTIVE SOURCING) ---
    ...
    try {
      ...
      try {
        ...
        if (poRes.data && poRes.data.po_number) {
          const templateRes = await axios.get(`${API_BASE_URL}/purchase-orders/${poRes.data.po_number}/template`);
  ...
  const handleSaveEdits = async () => {
    ...
    try {
      ...
      if (poRes.data && poRes.data.po_number) {
        await axios.put(`${API_BASE_URL}/purchase-orders/${poRes.data.po_number}/template`, {
          html_content: element.innerHTML
        });
        setAlert({ type: 'success', message: `Saved custom edits for PO ${poRes.data.po_number} successfully!` });
      } else {
        setAlert({ type: 'error', message: "Cannot save. Purchase Order has not been formally generated yet." });
      }
    } catch (err) {
  ...
  const primaryLine = historyPoItems[0] || {};
