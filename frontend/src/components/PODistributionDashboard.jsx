...
import { Download, FileText, Edit3, Stamp, ArrowRight, CheckCircle2, AlertCircle, X, Save } from 'lucide-react';
...
export default function PODistributionDashboard({ currentUser }) {
  ...
  const openPoTemplate = async (po) => {
    ...
    setAlert(null);
    setLoading(true);
    setSavedHtml(null); // Reset the saved HTML
    try {
      // 🎯 CHECK IF A SAVED TEMPLATE EXISTS FOR THIS PO
      try {
        const templateRes = await axios.get(`${API_BASE_URL}/purchase-orders/${po.po_number}/template`);
        if (templateRes.data && templateRes.data.html_content) {
          setSavedHtml(templateRes.data.html_content);
        }
      } catch (err) {
        // Ignore errors here; it just means no custom template has been saved yet
      }
  ...
  const handleSaveEdits = async () => {
    ...
    setSaveLoading(true);
    try {
      await axios.put(`${API_BASE_URL}/purchase-orders/${selectedPo.po_number}/template`, {
        html_content: element.innerHTML
      });
      setAlert({ type: 'success', message: `Saved custom edits for PO ${selectedPo.po_number} successfully!` });
    } catch (err) {
      setAlert({ type: 'error', message: "Failed to save template edits to database." });
    } finally {
      setSaveLoading(false);
    }
  };

  // 🎯 FLAWLESS NATIVE VECTOR PDF DOWNLOAD
    ...
    <div className="space-y-6 pb-12">
      ...
      <style>{`
        @media print {
          ...
          *, *::before, *::after { overflow: visible !important; position: static !important; }
          ...
          #printable-po {
            ...
            width: 210mm !important; max-width: 210mm !important; padding: 15mm !important;
            margin: 0 !important; border: none !important; box-shadow: none !important; box-sizing: border-box !important;
          }
          table { width: 100% !important; table-layout: fixed !important; }
          th, td, p { overflow-wrap: break-word !important; word-wrap: break-word !important; }
          .print\\:hidden, .seal-footer { display: none !important; }
            ...
            <Card className="bg-white border-slate-200 shadow-sm overflow-hidden relative print:border-none print:shadow-none">
              ...
              <div className="bg-slate-900 text-white p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-3 print:hidden">
                <div className="flex items-center space-x-2.5">
                  ...
                  <span className="text-xs uppercase font-bold tracking-tight text-amber-50">Live Editable Canvas • Click text to modify</span>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <button 
                    onClick={handleSaveEdits} 
                    disabled={saveLoading}
                    className="bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-all flex items-center justify-center space-x-1.5 px-3.5 py-2 text-xs font-bold shadow-xs w-full sm:w-auto"
                  >
              ...
              <div className="overflow-x-auto custom-scrollbar bg-slate-200/50 p-2 sm:p-4 rounded-b-xl border-t border-slate-200 flex justify-center print:p-0 print:border-none print:bg-white">
                ...
                >
                  ...
                  {/* If a saved template exists, render it. Otherwise, render dynamically */}
                  {savedHtml ? (
                    ...
                    <>
                      ...
                      {selectedPo.category === 'GOODS' && (
                        <div className="space-y-4 text-xs text-slate-800 leading-relaxed relative z-10 w-full">
                          ...
                          <div className="space-y-1 avoid-break w-full" contentEditable="true">
